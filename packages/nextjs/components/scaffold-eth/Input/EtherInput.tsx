...
export const EtherInput = ({ value, name, placeholder, onChange, disabled }: CommonInputProps) => {
  ...
  return (
    <InputBase
      ...
      disabled={disabled}
      prefix={<span className="pl-4 -mr-2 text-[#3C3E4E] self-center">{usdMode ? "$" : "Ξ"}</span>}
      suffix={
        <button
          className={`btn btn-primary h-[2.2rem] min-h-[2.2rem] text-white ${nativeCurrencyPrice > 0 ? "" : "hidden"}`}
          onClick={toggleMode}
